    ...
    return (
        <Card
            ...
            sx={{
                ...
                borderColor: title === "🌟 Topper" ? vibrantPalette[0] : title.startsWith("🥇") ? vibrantPalette[1] : vibrantPalette[7],
                boxShadow: 1,
                display: 'flex',
        ...
        >
            <CardContent sx={{ flexGrow: 1 }}>
                ...
                <Typography variant="body2" color="text.secondary" mt={1} sx={{ display: 'block', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                    {student.subjects.map(sub => `${sub.subject}: ${sub.score} (${sub.grade})`).join(', ')}
                </Typography>
            </CardContent>
            <Box sx={{ p: 1, display: 'flex', justifyContent: 'flex-end' }}>
                <Button size="small" onClick={() => onView(student)}>View Details</Button>
            </Box>
        </Card>
    );
};
                ...
                <Container maxWidth="lg" sx={{ mt: 4, mb: 6 }}>
                    ...
                    {report && (
                        <>
                            ...
                            <Paper elevation={3} sx={{ p: 2, mb: 3 }}>
                                <Tabs value={topStudentsTab} onChange={(event, newValue) => setTopStudentsTab(newValue)} aria-label="top students tabs">
                                    <Tab label="Topper" />
                                    <Tab label="Top 3" disabled={!report.top3 || report.top3.length < 3} />
                                    <Tab label="Bottom 3" disabled={!report.bottom3 || report.bottom3.length < 3} />
                                </Tabs>
                                {topStudentsTab === 0 && report.topper && (
                                    <Box display="flex" justifyContent="space-around" alignItems="center">
                                        <StudentCard title="🌟 Topper" student={report.topper} onView={handleViewStudentDetails} />
                                        <Button startIcon={<PictureAsPdfIcon />} onClick={downloadTopperPdf} size="small">
                                            Download PDF
                                        </Button>
                                    </Box>
                                )}
                                {topStudentsTab === 1 && report.top3 && report.top3.length > 0 && (
                                    <Box>
                                        <Grid container spacing={2} justifyContent="center" mb={2}>
                                            {report.top3.map((student, index) => (
                                                <Grid item key={student.name}>
                                                    <StudentCard title={`🥇 Rank ${index + 1}`} student={student} onView={handleViewStudentDetails} />
                                                </Grid>
                                            ))}
                                        </Grid>
                                        <Button startIcon={<PictureAsPdfIcon />} onClick={downloadTop3Pdf} size="small">
                                            Download Top 3 PDF
                                        </Button>
                                    </Box>
                                )}
                                {topStudentsTab === 2 && report.bottom3 && report.bottom3.length > 0 && (
                                    <Box>
                                        <Grid container spacing={2} justifyContent="center" mb={2}>
                                            {report.bottom3.map((student, index) => (
                                                <Grid item key={student.name}>
                                                    <StudentCard title={`📉 Bottom ${index + 1}`} student={student} onView={handleViewStudentDetails} />
                                                </Grid>
                                            ))}
                                        </Grid>
                                        <Button startIcon={<PictureAsPdfIcon />} onClick={downloadBottom3Pdf} size="small">
...
<Dialog open={openDetailsDialog} onClose={handleCloseDetailsDialog} fullWidth maxWidth="sm">
    <DialogTitle>{selectedStudentDetails?.name} - Details</DialogTitle>
    <DialogContent>
            ...
            <Table aria-label="student details">
                ...
                <TableBody>
                    ...
                    <TableRow>
                        ...
                        <TableCell align="right"><strong>{selectedStudentDetails.total}</strong></TableCell>
                        <TableCell align="right"><strong>{selectedStudentDetails.overallGrade}</strong></TableCell>
                    </TableRow>
                    <TableRow>
                        <TableCell><strong>Percentage</strong></TableCell>
                        <TableCell align="right"><strong>{selectedStudentDetails.percentage?.toFixed(2)}%</strong></TableCell>
                        <TableCell align="right"></TableCell>
                    </TableRow>
                    <TableRow>
                        <TableCell><strong>Pass/Fail</strong></TableCell>
                        <TableCell align="right"><strong>{selectedStudentDetails.passed ? <Typography color="success">Passed</Typography> : <Typography color="error">Failed</Typography>}</strong></TableCell>
                        <TableCell align="right"></TableCell>
                    </TableRow>
                </TableBody>
            </Table>
